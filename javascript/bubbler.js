	...
	initialize: function ( canvasDiv, canvas) {
		...
		if (BubblerPrefs.animation) {
			this.background.initialize(this.ctx, this.backgroundImage, 
							[{direction: 1, sources: ["clownfish_right.png", "greenfish_right.png"]},
							 {direction: -1, sources: ["clownfish_left.png", "greenfish_left.png"]}],"");
		}
		else {
...
handleUpgradeTap: function () {
	...
	endGame: function(playAgain) {
		if (!this.gameOver) {
			...
			ScoreList.addScore({name:name, points: this.score});
